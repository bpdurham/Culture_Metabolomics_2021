...
dat = read.table(file='All_PA_features-2022.csv', header = T, row.names = NULL, sep=',', stringsAsFactors = FALSE)
unique(dat$notes)
...
fresh_sum = colSums(subset_chry[which(rownames(subset_chry) == 'Chryso.116' | rownames(subset_chry) == 'Chryso.P3'),], na.rm=TRUE)
...
new_matrix_noNA <- matrix_noNA[ order(row.names(matrix_noNA)), ]
jacd_ordered <- vegdist(new_matrix, method="jaccard", na.rm=TRUE)
fviz_dist(jacd_ordered, order=FALSE, lab_size=6)
jacd_ordered_noNA <- vegdist(new_matrix_noNA, method="jaccard", na.rm=TRUE)
fviz_dist(jacd_ordered_noNA, order=FALSE, lab_size=6)


pdf(file='dist_matrices.pdf', height=8, width=8)
par(mfrow=c(1,2))
fviz_dist(jacd_ordered, order=FALSE, lab_size=6)
fviz_dist(jacd_ordered_noNA, order=FALSE, lab_size=6)
dev.off()
...
#make a heatmap
#cleanup the metabolites names

colnames(matrix) = gsub("-HILIC", "", colnames(matrix))
colnames(matrix) = gsub("-RP", "", colnames(matrix))
colnames(matrix_noNA) = gsub("-HILIC", "", colnames(matrix))
colnames(matrix_noNA) = gsub("-RP", "", colnames(matrix))
...
heatmap.2(matrix, trace = "none", dendrogram = "both", scale='none', Rowv=as.dendrogram(clust), Colv=as.dendrogram(clust2), key = F,
          ...
          margins = c(12,4), cexRow=0.4, cexCol = 0.35)
